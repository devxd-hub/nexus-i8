...
export async function fetchMemberByIdentifier(
  ...
  try {
    ...
    try {
      ...
      if (API_BASE === '' && (!ct.includes('application/json') && res.status === 200)) {
        // Vite SPA HTML fallback was triggered; fetch direct from port 3001
        res = await fetch(`http://localhost:3001${targetUrl}`, {
          headers: { Accept: 'application/json' },
        });
      }
    } catch (fetchErr) {
