...
// Global cache to avoid recomputing Poisson depth fields across re-renders
...
export const MetallicPaint: React.FC<MetallicPaintProps> = ({
  ...
  const initGL = useCallback(() => {
    ...
    if (!gl) {
      console.warn('WebGL2 not supported for MetallicPaint');
      return false;
    }
  ...
  useEffect(() => {
    ...
    const side = Math.min(Math.round(512 * Math.min(dpr, 2)), 1024);
    canvas.width = side;
    canvas.height = side;
  ...
  useEffect(() => {
    ...
    if (processedCache.has(imageSrc)) {
      const cached = processedCache.get(imageSrc)!;
    ...
    img.onload = () => {
      try {
        ...
        processedCache.set(imageSrc, imgData);
        uploadTexture(imgData);
        setTextureReady(true);
      } catch (err) {
        console.error('Error processing MetallicPaint image:', err);
      }
    };
    img.onerror = (err) => {
  ...
  useEffect(() => {
    ...
    const render = (time: number) => {
      ...
      // Check hover state if controlled
      if (isHoveredRef.current !== undefined) {
        if (isHoveredRef.current) {
          lastActiveTime = time;
          rafRef.current = requestAnimationFrame(render);
        } else {
          // Allow loop to settle smoothly for ~500ms after mouse leave
          if (time - lastActiveTime < 500) {
            rafRef.current = requestAnimationFrame(render);
          } else {
            isLooping = false;
            rafRef.current = null;
  ...
  }, [ready, textureReady, isHovered]);

  return (
    <canvas
      ref={canvasRef}
      className={`block h-full w-full object-contain pointer-events-none select-none ${className}`}
      aria-hidden="true"
    />
  );
};
