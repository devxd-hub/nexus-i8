...
import { Terminal, MapPin, ShieldCheck } from 'lucide-react';
import { useStoryScroll } from './ScrollStoryContext.tsx';

interface StoryChapter09HorizonProps {
  onRouteChange: (route: AppRoute) => void;
}

export const StoryChapter09Horizon: React.FC<StoryChapter09HorizonProps> = ({ onRouteChange }) => {
  const { smoothProgress } = useStoryScroll();

  // Scroll-driven visual transformations for Chapter 09 (0.90 -> 1.00 timeline)
  const finalCardScale = useTransform(smoothProgress, [0.90, 0.98, 1.0], [0.97, 1, 1]);
  const closingXRotate = useTransform(smoothProgress, [0.88, 1.0], [0, 90]);

  return (
    <section
      id="chapter-09-horizon"
      className="relative py-20 sm:py-28 md:py-36 bg-[#F3EEE5] text-[#0A0A09] overflow-hidden"
    >
      <Container className="relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 mb-16">
          <div className="p-8 bg-[#FAF6F0] border border-[rgba(10,10,9,0.14)] space-y-4 shadow-xs rounded-[2px]">
        ...
        {/* Final Story Resolution & Primary Action Triggers with Scroll Scaling */}
        <motion.div
          style={{ scale: finalCardScale }}
          className="p-8 sm:p-12 bg-[#0A0A09] text-[#F3EEE5] rounded-[2px] border border-[rgba(243,238,229,0.16)] shadow-lg text-center space-y-8"
        >
          <div className="max-w-2xl mx-auto space-y-4">
            <motion.div
              style={{ rotate: closingXRotate }}
