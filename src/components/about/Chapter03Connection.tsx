...
import { Sparkles, Network, Terminal, Cpu, Palette, Radio, Search, Box } from 'lucide-react';
...
interface DisciplineNode {
  ...
  tools: string[];
  x: number; // percentage coordinates for canvas
  y: number;
}
...
export const Chapter03Connection: React.FC = () => {
  ...
  return (
    ...
    >
      <Container className="relative z-10">
        ...
        {/* Interactive Centerpiece Disciplines Network */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center mb-16">
          {/* Interactive Network Visual Canvas */}
          <div className="lg:col-span-8 relative aspect-[4/3] sm:aspect-[16/10] bg-[#FAF6F0] border border-[rgba(10,10,9,0.18)] shadow-sm rounded-[2px] p-6 flex items-center justify-center overflow-hidden">
            ...
            {DISCIPLINE_NODES.map((node) => {
              ...
              return (
                <button
                  ...
                  className={`absolute z-20 flex items-center gap-2 p-2 sm:px-3 sm:py-2 rounded-[2px] transition-all duration-250 cursor-pointer shadow-xs border ${
          ...
          <div className="lg:col-span-4 p-8 bg-[#FAF6F0] border border-[rgba(10,10,9,0.16)] rounded-[2px] shadow-xs space-y-6">
            ...
            <div className="space-y-2 pt-2 border-t border-[rgba(10,10,9,0.08)]">
              <span className="font-dosis text-[11px] font-bold uppercase tracking-[0.2em] text-[#66615A] block">
                STUDIO TOOLS & METHODS
              </span>
              <div className="flex flex-wrap gap-1.5">
                {activeNode.tools.map((tool) => (
                  <span
                    key={tool}
                    className="px-2 py-0.5 bg-white border border-[rgba(10,10,9,0.1)] text-[#66615A] font-dosis text-[11px] tracking-wider"
                  >
                    {tool}
                  </span>
                ))}
