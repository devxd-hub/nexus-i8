...
export const EditorialCoordinatorHero: React.FC<EditorialCoordinatorHeroProps> = ({
  ...
  return (
    <div
      ...
      className="relative w-full min-h-[92vh] lg:min-h-screen py-16 sm:py-24 bg-[#F3EEE5] text-[#0A0A09] flex items-center justify-center border-b border-[#0A0A09]/15 px-4 sm:px-8 md:px-12 lg:px-16 overflow-hidden select-none"
    >
      {/* Background Watermark Numerals */}
      <div
        className="absolute top-1/2 -translate-y-1/2 right-4 sm:right-12 font-dosis font-bold text-[24vw] text-[#0A0A09]/[0.025] pointer-events-none select-none tracking-tighter"
        aria-hidden="true"
      >
        {paddedIndex}
      </div>
      ...
      >
        ...
        >
          <div
            ...
            className="group relative w-full aspect-[4/5] sm:aspect-[16/11] lg:aspect-[4/3] bg-[#E3DDD1] overflow-hidden border-2 border-[#0A0A09] shadow-sm cursor-pointer"
            role="button"
            tabIndex={0}
        ...
        >
          ...
          <div className="flex items-center gap-3">
            <span className="font-fraunces font-bold text-3xl sm:text-4xl text-[#EF5A2A] tracking-tighter">
              {paddedIndex}
            </span>
            <div className="h-4 w-[1px] bg-[#0A0A09]/20" />
            <span className="font-dosis text-xs font-bold tracking-[0.24em] text-[#66615A] uppercase">
          ...
          <motion.div
            ...
            className="space-y-1 bg-[#F3EEE5]/90 backdrop-blur-[2px] lg:p-6 lg:border lg:border-[#0A0A09]/15 lg:shadow-md"
          >
            <span className="font-dosis font-bold text-xs sm:text-sm tracking-[0.26em] text-[#EF5A2A] uppercase block">
              {member.role}
            </span>

            <h3 className="font-fraunces font-bold text-3xl sm:text-5xl lg:text-5xl xl:text-6xl text-[#0A0A09] tracking-tight leading-[0.95] uppercase">
              {firstName}
              {lastName && (
                <>
                  <br />
                  <span className="text-[#0A0A09]/85">{lastName}</span>
                </>
              )}
            </h3>

            {member.bio && (
              <p className="font-bitter text-sm sm:text-base text-[#0A0A09]/80 pt-4 leading-relaxed border-t border-[#0A0A09]/10">
                {member.bio}
              </p>
            )}
            ...
            <div className="pt-4 flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() => onSelectMember(member)}
                className="inline-flex items-center gap-2 px-3 py-1.5 bg-[#0A0A09] text-white hover:bg-[#EF5A2A] transition-colors duration-200 font-dosis font-bold text-xs tracking-[0.2em] uppercase border border-[#0A0A09] cursor-pointer"
              >
                <span>DOSSIER</span>
                <ArrowUpRight className="w-3.5 h-3.5 stroke-[2.5]" />
              </button>
