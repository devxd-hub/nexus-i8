...
export const SectionHeader: React.FC<SectionHeaderProps> = ({
  ...
  if (align === 'between') {
    return (
      <div
        id={id}
        className={`flex flex-col md:flex-row md:items-end justify-between gap-6 pb-8 border-b border-[rgba(245,239,230,0.10)] ${className}`}
      >
        <div className="max-w-3xl">
          <SectionLabel number={labelNumber} label={labelText} className="mb-3" />
          <h2 className="nexus-heading-xl text-[#F5EFE6] mt-1">{title}</h2>
  ...
  return (
    <div
      id={id}
      className={`pb-8 border-b border-[rgba(245,239,230,0.10)] ${className}`}
    >
      <SectionLabel number={labelNumber} label={labelText} className="mb-3" />
      <h2 className="nexus-heading-xl text-[#F5EFE6] mt-1">{title}</h2>
      {subtitle && <p className="nexus-body mt-3 max-w-2xl">{subtitle}</p>}
