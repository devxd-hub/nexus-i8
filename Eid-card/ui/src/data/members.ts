...
export function normalizeMemberJson(input: unknown): TeamMember {
  ...
  const qrUrl = String(raw.qrUrl || raw.url || (raw.uniqueId ? `/memberID/${slug}/${raw.uniqueId}` : `/team/${slug}`));
...
export function getMemberShareUrl(member: TeamMember): string {
  ...
  if (isCanonical) {
    return `${origin}/team/${member.slug}`;
  }

  // Deep-link for custom or user-modified member card
  try {
    const jsonStr = JSON.stringify(member);
    return `${origin}/memberID/${member.slug}/${member.id}?json=${encodeURIComponent(jsonStr)}`;
  } catch {
    return `${origin}/team/${member.slug}`;
  }
}
